...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, modals, optimizer=None, train=False, dataset='IEMOCAP'):
    ...
    for data in dataloader:
        ...
        if args.multi_modal and args.mm_fusion_mthd=='gated':
            ...
            log_prob, e_i, e_n, e_t, e_l, emotions_feat = model([textf1,textf2,textf3,textf4], qmask, umask, lengths, acouf, visuf, epoch)
            # print("one iteration")
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_L', type=int, default=8, help='num_hyperconvs')
    ...
    for e in range(n_epochs):
        ...
        if best_fscore == None or best_fscore < test_fscore:
            ...
            print(f"New best model saved with fscore: {test_fscore}")
        if args.tensorboard:
        ...
        print('epoch: {}, train_loss: {}, train_acc: {}, train_fscore: {}, test_loss: {}, test_acc: {}, test_fscore: {}, Learning Rate: {}, time: {} sec'.\
                format(e+1, train_loss, train_acc, train_fscore, test_loss, test_acc, test_fscore, current_lr, round(time.time()-start_time, 2)))
        if (e+1)%10 == 0:
